...
function getFromCache(key: string): { position: GeoPoint; address: string } | null {
  try {
...
function addToCache(key: string, data: { position: GeoPoint; address: string }) {
  try {
    const raw = localStorage.getItem(CACHE_KEY);
...
export const getUserZipCode = async (point: GeoPoint, apiKey: string): Promise<string | null> => {
  try {
...
export const geocodeAddress = async (query: string, apiKey: string, userZip?: string): Promise<{ position: GeoPoint, address: string } | null> => {
  // Construct query key
...
export const calculateOptimalSequence = async (
  ...
  validDestinations.forEach((addr, index) => {
    if (addr.location) {
      url.searchParams.append(`destination${index + 1}`, `${addr.id};${addr.location.lat},${addr.location.lng}`);
    }
  });
...
export const getRouteShape = async (
    ...
    if (sortedWaypoints.length > 0) {
        ...
        intermediates.forEach(wp => {
             if(wp.location) v8Url.searchParams.append('via', `${wp.location.lat},${wp.location.lng}`);
        });

        if (last.location) {
            v8Url.searchParams.append('destination', `${last.location.lat},${last.location.lng}`);
        }
