...
import { loadGoogleModules, getAccessToken, openGooglePicker, fetchSheetRows } from '../services/googleSheetService';
...
const GOOGLE_CLIENT_ID = '377676797720-gue2trd92glfihma88lsm813je9u51al.apps.googleusercontent.com';
  ...
  const handleGoogleAction = async (action: 'PICK' | 'SYNC') => {
    ...
    try {
      ...
      if (action === 'PICK' || !targetSheetId) {
        ...
        // Save selection
        const newConfig = { spreadsheetId: targetSheetId, spreadsheetName: targetSheetName };
      ...
      alert('Failed to connect to Google Sheets. Ensure your account has access.');
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  return (
    <div className="w-full bg-white border-r border-gray-200 flex flex-col h-full shadow-xl z-10 relative">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                 <label className="block text-sm font-medium text-gray-700">Import Source</label>
                 {sheetConfig && (
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            >
                {importStatus === ImportStatus.PARSING ? 'Parsing...' : 'Parse Text'}
            </button>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-800">Stops ({addresses.length})</h2>
            {addresses.length > 0 && (
                <button 
              ...
              return (
                ...
                >
                  <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                      addr.sequenceOrder ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'
                  }`}>
                     {isLoading ? (
                  ...
                  <div className="flex-1 min-w-0">
                      {addr.name && (
                          <p className="text-gray-900 font-bold truncate">{addr.name}</p>
                      )}
                      <p className={`${addr.name ? 'text-gray-500 text-xs' : 'text-gray-900 font-medium'} truncate`}>
      ...
      <div className="p-6 bg-gray-50 border-t border-gray-200 space-y-3">
         {isGeocoding && (
             <div className="text-xs text-center text-blue-600 mb-2">Geocoding addresses...</div>
         )}
         <button
          onClick={onOptimize}
          disabled={isOptimizing || isGeocoding || addresses.length < 2 || !userLocation}
