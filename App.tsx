...
import { geocodeAddress, calculateOptimalSequence, getRouteShape, getUserZipCode } from './services/hereService';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const geocodePending = async () => {
      ...
      const pendingAddresses = addresses.filter(a => a.isGeocoding && !a.location);
      ...
      try {
          for (const addr of pendingAddresses) {
             ...
             try {
                 const result = await geocodeAddress(addr.originalText, apiKey, userZip || undefined);
                 ...
                 setAddresses(prev => prev.map(a => a.id === addr.id ? { ...a, isGeocoding: false } : a));
  ...
  const handleOptimize = async () => {
    ...
    try {
      // 1. Solve TSP
      const { sortedAddresses } = await calculateOptimalSequence(userLocation, addresses, apiKey);
      ...
      alert(error.message || "Could not optimize route. Please check your API Key and try again.");
    } finally {
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-gray-100 overflow-hidden">
      ...
      {showKeyModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 backdrop-blur-sm px-4">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-md w-full">
            <h2 className="text-2xl font-bold mb-2 text-gray-900">Enter HERE Maps API Key</h2>
            <p className="text-gray-600 mb-6 text-sm">
                This application uses HERE Maps for geocoding, mapping, and route optimization. 
                You can get a free Freemium key from <a href="https://platform.here.com/" target="_blank" className="text-blue-600 underline">developer.here.com</a>.
            </p>
            <input
              type="text"
      ...
      <div className="flex-1 flex relative overflow-hidden">
        {/* Sidebar Container */}
        <div className={`w-full md:w-96 bg-white z-20 h-full flex-shrink-0 flex flex-col ${mobileTab === 'list' ? 'block' : 'hidden md:block'}`}>
          <Sidebar
            addresses={addresses}
